...
use bevy::{pbr::wireframe::{Wireframe, WireframeConfig, WireframePlugin}, prelude::*, render::{mesh::{Indices, PrimitiveTopology}, render_asset::RenderAssetUsages, settings::{RenderCreation, WgpuFeatures, WgpuSettings}, RenderPlugin}, sprite::{MaterialMesh2dBundle, Mesh2dHandle} 
};
use bevy_pancam::{PanCam, PanCamPlugin};
use rand::Rng;
use voronator::{delaunator::{Coord, Point}, polygon::Polygon, CentroidDiagram, VoronoiDiagram};
...
fn startup_system(mut commands: Commands,
    ...
    let diagram = generate_diagram(
        Point { x: 0., y: 0. }, 
        Point { x: window.resolution.physical_width() as f64, y: window.resolution.physical_height() as f64 }
        , 15.
    );
    ...
    for (polygon, center) in zip(diagram.cells().iter(), diagram.sites.iter()) {
        ...
        let points = polygon.points().iter().map(|point| HashablePoint::from(*point)).collect::<Vec<HashablePoint>>();
    ...
    let poly_meshes = generate_voronoi_mesh(&diagram);
    ...
    for mesh in poly_meshes{
        commands.spawn((MaterialMesh2dBundle{
            ...
            transform: Transform::from_translation(Vec3::new(-width as f32, -height as f32, 0.)),
            ..Default::default()
        }, Wireframe));
    }
...
fn generate_points_poisson(min: Point, max: Point, radius: f64) -> Vec<Point>{
    let sample = Poisson2D::new().with_dimensions([max.x, max.y], radius,).generate();
    let points = sample.iter().map(|p| Point{x: p[0], y: p[1]}).collect::<Vec<Point>>();
    points
}

fn generate_polygon_outline_mesh(polygons: &[Polygon<Point>], width: f64, height: f64) -> Vec<Mesh>{
...
fn generate_voronoi_mesh(diagram: &VoronoiDiagram<Point>) -> Vec<Mesh> {
    let mut meshes: Vec<Mesh> = Vec::new();
...
fn generate_centroid_mesh(diagram: &CentroidDiagram<Point>) -> Vec<Mesh> {
    let meshes: Vec<Mesh> = Vec::new();
    for polygon in diagram.cells.iter() {
        let mut points = Vec::new();
        for point in polygon.points() {
            points.push([point.x as f32, point.y as f32, 0.]);
        }
        let mut indices = Vec::new();
        for i in 1..points.len() - 1 {
            indices.push(0);

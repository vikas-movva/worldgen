...
async function main() {
  ...
  const ops = [{ mode: "Raise", center_cell: centerCell, target_cell: centerCell, radius: 5, strength: 0.5, cells: [] }];
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!(hPatch instanceof Uint8Array) || hPatch.length !== N) {
      console.error(`FAIL: edit_heightmap_h returned ${hPatch?.constructor?.name} len=${hPatch?.length}, expected Uint8Array[${N}]`);
      process.exit(1);
    }
  }

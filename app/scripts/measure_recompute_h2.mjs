...
async function main() {
	...
	for (let i = 0; i < 9; i++) {
		const t0 = performance.now();
		const _result = recompute_dependents_h2({});
	...
	console.log(
		`recompute_dependents_h  (serde return):     ${median(hTimes).toFixed(2)}ms median`,
	);
	console.log(
		`recompute_dependents_h2 (TypedArray return): ${median(h2Times).toFixed(2)}ms median`,
	);
	console.log(`\nVerification:`);
	...
	console.log(
		`  coastline is Uint8Array: ${r2.coastline instanceof Uint8Array}`,
	);
	...
	console.log(
		`Saved: ${(median(hTimes) - median(h2Times)).toFixed(2)}ms per recompute_dependents call`,
	);
}

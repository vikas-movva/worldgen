...
import type { TimelineEvent, WarOutcome } from "../core/api";
import type { Burg, Culture, Religion, State } from "../state/types";
...
function formatEventPayload(ev: TimelineEvent): string | null {
	...
	switch (ev.kind) {
		case "War": {
			...
			const cellsStr =
				cells.length > 0
					? ` — ${cells.length} cell${cells.length === 1 ? "" : "s"} conquered`
					: "";
		...
		case "Conquer":
			return p.kind === "Conquer"
				? `claimed ${p.data.payload.cells.length} cell${p.data.payload.cells.length === 1 ? "" : "s"}`
				: "conquered territory";
		case "Secession":
		...
		case "Plague":
			return p.kind === "PopScalar"
				? `plague (factor ${p.data.factor.toFixed(2)})`
				: "plague";
		case "GoldenAge":
			return p.kind === "PopScalar"
				? `golden age (growth ×${p.data.factor.toFixed(2)})`
				: "golden age";
		case "Schism":
			if (p.kind === "Schism") {
		...
		case "Raise":
			return p.kind === "Raise"
				? `raised army (size ${p.data.army_size}) at cell ${p.data.cell}`
				: "army raised";
		case "March":
			return p.kind === "March"
				? `army marched to cell ${p.data.cell}`
				: "army marched";
		case "Disband":
			return "army disbanded";
		case "Raze":
			return p.kind === "Raze"
				? `burg razed on cell ${p.data.cell}`
				: "burg razed";
		case "Dissolve":
...
function buildAttrs(
	...
	if (kind !== "burg") {
		rows.push({
			label: "Color",
			value: hexCss((entity as { color: number }).color),
		});
	}
	...
	if (kind === "state") {
		rows.push(
			...
			{
				label: "Tax rate",
				value: s.tax_rate != null ? String(s.tax_rate) : "?",
			},
	...
	const f = (
		entity as { founded_year?: number; dissolved_year?: number | null }
	).founded_year;
...
export function EntityInspector(): React.ReactElement | null {
	...
	const { kind, entity, attrs, events } = useMemo(() => {
		if (!selectedEntity)
			return { kind: null, entity: null, attrs: [], events: [] };
		...
		if (k === "state" || k === "province") {
			...
			const vec =
				k === "culture" ? culturesResult?.cultures : culturesResult?.religions;
			ent = vec?.find((e) => e.id === i) ?? null;
		}
		...
		>
			...
			>
				...
				{events.length > 0 ? (
					...
					>
						No chronicle events for this entity yet. The timeline engine (Phase
						5.1) populates this tab with the entity's succession / schism /
						conquest events once <code>generateTimeline</code>

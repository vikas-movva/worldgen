...
/**
 * Build a flat per-cell RGBA Uint8Array (length N*4) for the data-texture
 * biome layer. `biome[i]` indexes `BIOME_COLORS`; index is clamped to range.
 */
export function buildBiomeTextureData(biome: Uint8Array | number[]): Uint8Array {
...
/**
 * Build a flat per-cell RGBA Uint8Array (length N*4) for the data-texture
 * terrain layer, colored by height via `heightColor`. Mirrors the GLSL gradient
 * so the mesh-shader and the texture-based paths agree.
 */
export function buildHeightTextureData(h: Uint8Array | number[]): Uint8Array {
  const n = h.length;
  const out = new Uint8Array(n * 4);
  for (let i = 0; i < n; i++) {
